...
### Late 09-23: Google drafting + calendar built; blocked on one admin-console step

- Rez's ruling: the drafting and calendar claims stay. Fact check on the box: Arlo on OpenClaw has NO Gmail/Calendar tools in-harness (tools profile "full", no google skill); email work runs through `/root/.meridian/arlo/bin/arlo-gmail.py` (service account, scopes gmail.readonly + gmail.send) via shell and host timers. Calendar has never worked on any harness: `arlo-calendar.py` has returned `calendar_not_authorized` since it was written because calendar scope was never delegated.
- Built and merged (main `d2f0537`): `examples/arlo/SKILLS/google` 0.2.0-sa (gmail_recent/search/read/get/pending/draft/send, gcal_today/upcoming/schedule) + `examples/arlo/bin/arlo-gmail.py` (new `draft` and side-effect-free `scopes` subcommands, home-relative paths via `ARLO_HOME`) + `examples/arlo/bin/arlo-calendar.py` (Denver-aware today, RW scope with RO fallback, `schedule`). Deployed to the BENCH home only (`/root/.meridian-bench/arlo/{bin,SKILLS/google}`); the production home's scripts are untouched. Certify `exec` probe kind + `--only` (partial, never a certification); Arlo's manifest has `google.live.gmail` (green now), `google.live.drafts`, `google.live.calendar` (red until scopes).
- Live scope check (`arlo-gmail.py scopes`): gmail.readonly true, gmail.send true, gmail.compose false, gmail.settings.basic false, calendar.events false, calendar.readonly false.
- BLOCKER (Rez, ~2 min): Google Workspace Admin > Security > API controls > Domain-wide delegation > client id `118376428827284284551` (aterna-agents@arlo-access-486417.iam.gserviceaccount.com): keep gmail.readonly + gmail.send, ADD https://www.googleapis.com/auth/gmail.compose, https://www.googleapis.com/auth/calendar.events, optionally https://www.googleapis.com/auth/gmail.settings.basic. Then: `certify --only google.live.drafts google.live.calendar` (free) from `/root/meridian-bench/dev-tree/bin/meridian`, then `scripts/ops/release.sh 1.5.0-rc.3 --confirm telegram.operator` (if he says yes) ≈ $0.80.
- At cutover: copy `examples/arlo/bin/*.py` and `examples/arlo/SKILLS/google` into the production home (the `draft`/`scopes` subcommands are additive; the inbox monitor keeps working).
