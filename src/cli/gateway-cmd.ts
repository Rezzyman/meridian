...
export async function runGateway(opts: { port?: number; web?: boolean }): Promise<void> {
  ...
  await startGateway({
    ...
    timezone: tz,
    spendCaps: {
      dailyUsd: config.spend.dailyUsd,
      monthlyUsd: config.spend.monthlyUsd,
      perTurnUsd: config.spend.perTurnUsd,
      perRunUsd: config.spend.perRunUsd,
      maxPromptTokensPerTurn: config.spend.maxPromptTokensPerTurn,
    },
    toolNames: Object.keys(surface.tools),
    vapi,
    slack,
    discord,
    whatsapp,
