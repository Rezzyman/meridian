...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-449%20passing-brightgreen?style=for-the-badge" alt="449 tests passing">
...
为每个智能体设置一个模型密钥（`OPENROUTER_API_KEY`，或 `ANTHROPIC_API_KEY` /
`OPENAI_API_KEY` / `GROQ_API_KEY`，或把 `OLLAMA_BASE_URL` 指向本地模型）。
...
Meridian 目前接入了 **7 个通道**，并通过 CORTEX 实现跨通道记忆：
...
在最关键的通道上，这已是**与 Hermes（~7）持平**；OpenClaw 的长尾（~23）在广度上仍领先。
