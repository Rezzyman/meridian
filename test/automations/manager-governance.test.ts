...
import { AutomationManager, loadAutomationDefs, toolIsConsequential } from '../../src/automations/manager.js';
...
function fixture(frontmatter: string) {
  ...
  writeFileSync(join(automations, 'test.cron'), `---\n${frontmatter}\n---\nProduce a useful result.\n`);
  const home = {
  ...
  const config = makeConfig({ operator: { id: 'atanasio', name: 'Rez', channels: { telegram: ['123'] } } });
  const sent: string[] = [];
  const channels = new Map([['telegram', { name: 'telegram', start() {}, stop() {}, send: async (msg: { text: string }) => { sent.push(msg.text); } }]]);
...
function manager(f: ReturnType<typeof fixture>, tools?: ToolSet) {
  return new AutomationManager({ home: f.home, config: f.config, cortex: f.cortex, router: mockRouter(textModel('evidence-backed result')), logger: silentLogger, systemBase: 'system', channels: f.channels as never, tools, store: f.store });
}
...
describe('automation runtime enforcement', () => {
  it('shadow mode neither pushes nor encodes', async () => {
    ...
    const m = manager(f); m.start();
    const result = await m.fire('test', new Date('2026-07-31T14:00:00Z')); m.stop();
  ...
  it('refuses consequential tools outside execute tier', async () => {
    ...
    const m = manager(f, tools); m.start();
    const result = await m.fire('test', new Date('2026-07-31T14:00:00Z')); m.stop();
