...
describe('deterministic cron calculation', () => {
  it('calculates the next weekday instead of jumping to the next matching January', () => {
    ...
    assert.equal(
      nextCronOccurrence('0 8 * * 1-5', from, 'America/Denver').toISOString(),
      '2026-08-03T14:00:00.000Z',
    );
    assert.equal(
      nextCronOccurrence('30 17 * * 1-5', from, 'America/Denver').toISOString(),
  ...
  it('supports lists, ranges, steps, and six-field second schedules', () => {
    ...
    assert.equal(
      nextCronOccurrence('0 8 * * 1,2,3,4,5', friday, 'America/Denver').toISOString(),
      '2026-07-31T14:00:00.000Z',
    );
    assert.equal(
      nextCronOccurrence('*/5 * * * * *', friday, 'UTC').toISOString(),
      '2026-07-31T14:00:00.000Z',
    );
  });
